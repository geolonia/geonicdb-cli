...
The GeonicDB CLI (`geonic`) is a command-line client for managing [GeonicDB](https://geonicdb.geolonia.com/) Context Broker resources. It supports the **NGSI-LD** API and provides an interface similar to `aws` CLI — every API operation is available as a CLI command.
...
The CLI provides built-in help in wp-cli style:
...
Profiles allow you to manage multiple connection configurations (e.g., development, staging, production).
...
| `me` | Display current authenticated user and token expiry |
...
For details on GeonicDB authentication mechanisms, see [AUTH.md](./AUTH.md).
...
|---|---|---|
| `entities list` | List entities | `GET /ngsi-ld/v1/entities` |
| `entities get <id>` | Get an entity by ID | `GET /ngsi-ld/v1/entities/{id}` |
| `entities create [json]` | Create a new entity | `POST /ngsi-ld/v1/entities` |
| `entities update <id> [json]` | Update attributes (PATCH) | `PATCH /ngsi-ld/v1/entities/{id}/attrs` |
| `entities replace <id> [json]` | Replace all attributes (PUT) | `PUT /ngsi-ld/v1/entities/{id}/attrs` |
| `entities upsert [json]` | Create or update entities | `POST /ngsi-ld/v1/entities?options=upsert` |
| `entities delete <id>` | Delete an entity by ID | `DELETE /ngsi-ld/v1/entities/{id}` |

`entities list` supports filtering options: `--type`, `--id-pattern`, `--query`, `--attrs`, `--georel`, `--geometry`, `--coords`, `--spatial-id`, `--limit`, `--offset`, `--order-by`, `--count`, `--key-values`.
...
For details on subscription features, see [SUBSCRIPTIONS.md](./SUBSCRIPTIONS.md).

### registrations (reg) — Manage context source registrations
...
Supports: `--aggr-methods`, `--aggr-period`.
...
For details on the rule engine, see [REACTIVCORE_RULES.md](./REACTIVCORE_RULES.md).
...
For details on catalog integration, see [INTEGRATIONS.md](./INTEGRATIONS.md).
...
The CLI commands map directly to [NGSI-LD API endpoints](./API_NGSILD.md). For example:

| CLI Command | HTTP Method | API Endpoint |
|---|---|---|
| `geonic entities list` | GET | `/ngsi-ld/v1/entities` |
| `geonic entities create` | POST | `/ngsi-ld/v1/entities` |
| `geonic entities get <id>` | GET | `/ngsi-ld/v1/entities/{id}` |
| `geonic entities update <id>` | PATCH | `/ngsi-ld/v1/entities/{id}/attrs` |
| `geonic sub list` | GET | `/ngsi-ld/v1/subscriptions` |
| `geonic temporal entities list` | GET | `/ngsi-ld/v1/temporal/entities` |
| `geonic admin tenants list` | GET | `/admin/tenants` |

For complete API details, see [API_NGSILD.md](./API_NGSILD.md) and [AUTH.md](./AUTH.md).
